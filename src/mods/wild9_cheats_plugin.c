...
{
    char text[16] = "";
    return psx_mod_option_value(PKG_CHEATS, FEATURE_CHEATS,
                                option_id, text, sizeof(text)) &&
           strcmp(text, "true") == 0;
}
...
static void wild9_cheats_vblank(void)
{
    ...
    if (s_drench_infinite_health) {
        ...
        if (drench_health == 0x0010u || (drench_health > 0 && drench_health < 0x0011u)) {
...
PSX_MOD_CONSTRUCTOR(wild9_register_cheats_plugin)
{
    (void)psx_mod_register_activation_plugin(
        "wild9.cheats", wild9_cheats_activate);
    (void)psx_mod_register_vblank_plugin(
        "wild9.cheats", wild9_cheats_vblank);
}
